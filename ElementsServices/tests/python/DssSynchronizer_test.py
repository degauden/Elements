...
from ElementsServices.DataSync.DssSynchronizer import *

from fixtures.ConfigFilesFixture import *

class TestDssSynchronizer(object):

    def testNotImplemented(self):
        dependency = theDependencyConfig()
        connectionConfig = ConnectionConfiguration(theIrodsFrConfig())
        distantRoot = connectionConfig.distantRoot
        localRoot = connectionConfig.localRoot
        dependencyConfig = DependencyConfiguration(distantRoot, localRoot, dependency)
        mock = DssSynchronizer(connectionConfig, dependencyConfig)
