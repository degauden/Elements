...
ifdef BINARY_TAG
  BUILDSUBDIR := $(BUILD_PREFIX_NAME).$(BINARY_TAG)
else
  BUILDSUBDIR := $(BUILD_PREFIX_NAME)
endif
BUILDDIR := $(CURDIR)/$(BUILDSUBDIR)
...
ifneq ($(USE_NINJA),)
  ...
  BUILD_CMD := $(NINJA) -C $(BUILDSUBDIR) $(NINJAFLAGS)
#  BUILD_CMD := cd $(BUILDSUBDIR) && $(NINJA) $(NINJAFLAGS)
else
  BUILD_CONF_FILE := Makefile
  BUILD_CMD := $(MAKE) -C $(BUILDSUBDIR)
endif
