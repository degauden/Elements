#!/usr/bin/env python
"""
Small tool to generate the astrotools toolchain from a given SGSCMT.
"""
...
class AstroToolsGenerator(object):
    """
    Class wrapping the details needed to generate the toolchain file from SGSCMT.
    """
...
SGS_prepare_paths()"""
    ...
    __AA_projects__ = ("ROOT")

    __special_dirs__ = {"fftw": "fftw3",
                        "GCCXML":  "gccxml",
                        "Qt":  "qt"
                        }
    ...
    def __init__(self, sgscmt_root):
        """
        Prepare the instance.

        @param sgscmt_root: path to the root directory of a given SGSCMT version
        """
        self.sgscmt_root = sgscmt_root

    def __repr__(self):
        """
        Representation of the instance.
        """
        return "AstroToolsGenerator(%r)" % self.sgscmt_root

    @property
    def versions(self):
        """
        Extract the external names and versions from an installed SGSCMT.
        ...
        versions = {}
        # We extract the statements from the requirements file of the SGS_Configuration package
        req = open(os.path.join(self.sgscmt_root, "SGS_Configuration", "cmt", "requirements"))
    ...
    def _content(self):
        ...
        yield self.__header__ % versions.pop("SGS")
        ...
        for name in self.__AA_projects__:
            ...
            yield "SGS_AA_project(%-5s %s)" % (name, versions.pop(name))
        ...
        template = "SGS_external_package(%%-%ds %%-%ds %%-%ds)" % lengths
        ...
        for name in sorted(versions.keys(), key=packageSorting):
            # special case
            if name == "uuid":
                yield "if(NOT ${SGS_OS}${SGS_OS_VERS} STREQUAL slc6) # uuid is not distributed with SLC6"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2 or not os.path.exists(sys.argv[1]):
        print "Usage : %s <path to SGSCMT version>" % os.path.basename(sys.argv[0])
        sys.exit(1)
    print AstroToolsGenerator(sys.argv[1])
